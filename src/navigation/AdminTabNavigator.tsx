...
import { AdminSwipeNavigationProvider } from '../contexts/AdminSwipeNavigationContext';

const { width: screenWidth } = Dimensions.get('window');

// Fast and Smooth Swipeable Admin Navigator with ScrollView
const SwipeableAdminNavigator: React.FC = () => {
    const [currentIndex, setCurrentIndex] = useState(0);
    const scrollViewRef = useRef<ScrollView>(null);

    // Memoized screen configuration for performance
    const screens = useMemo(() => [
        { name: 'Dashboard', component: AdminDashboardScreen, key: 'dashboard' },
        { name: 'Orders', component: AdminOrdersScreen, key: 'orders' },
        { name: 'MenuManagement', component: MenuManagementScreen, key: 'menumanagement' },
    ...
    const currentTabName = screens[currentIndex]?.name || 'Dashboard';
    ...
    // Create optimized mock descriptors for admin icons
    const mockDescriptors = useMemo(() => {
        const descriptors: any = {};
        screens.forEach((screen) => {
            descriptors[screen.key] = {
                options: {
                    tabBarLabel: screen.name === 'MenuManagement' ? 'Menu' : screen.name,
                    tabBarIcon: ({ color, size }: { color: string; size: number }) => {
                        const iconName =
                            screen.name === 'Dashboard' ? 'stats-chart' :
                                screen.name === 'Orders' ? 'receipt' :
                                    screen.name === 'MenuManagement' ? 'restaurant' : 'person';
    ...
    return (
        ...
        >
            <View style={{ flex: 1, backgroundColor: '#fff' }}>
                ...
                {/* Fixed CustomTabBar with enhanced styling */}
                <View style={{
                    position: 'absolute',
                    bottom: 0,
                    left: 0,
